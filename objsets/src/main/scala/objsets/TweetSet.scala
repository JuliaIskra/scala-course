...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList =
    def addToList(list: TweetList, tweet: Tweet): TweetList =
      if list.isEmpty
      then Cons(tweet, list)
      else if list.head.retweets > tweet.retweets
      then Cons(list.head, addToList(list.tail, tweet))
      else Cons(tweet, list)

    def recur(set: TweetSet, acc: TweetList): TweetList =
      try
        val max = set.mostRetweeted
        recur(set.remove(max), addToList(acc, max))
      catch
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t =>
    var res = false
    google.foreach(k => res = res || t.text.contains(k))
    res)
